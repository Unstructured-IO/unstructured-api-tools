...
def test_supported_mimetypes(mimetype, filename, expected_status):
    ...
    # When there are more test fixtures - come back and add single file plus text input, multi file plus text input

    # If the client doesn't set a mimetype, we may just see application/octet-stream
    # Here we fall back to the file extension
    response = client.post(
        PROCESS_FILE_1_ROUTE, files=[("files", (filename, open(FILE_A, "rb"), "application/octet-stream"))]
    )
    assert(response.status_code == expected_status)
...
def test_reconfigure_supported_mimetypes():
    ...
    response = client.post(
        PROCESS_FILE_1_ROUTE, files=[("files", (FILE_A, open(FILE_A, "rb"), FILENAME_FORMATS[FILE_A]))]
    )
    ...
    response = client.post(
        PROCESS_FILE_1_ROUTE, files=[("files", (FILE_B, open(FILE_B, "rb"), FILENAME_FORMATS[FILE_B]))]
    )
    assert(response.status_code == 200)
